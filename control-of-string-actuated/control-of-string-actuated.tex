...
Problem involved into the following analysis can be schematized as shown in Figures~\ref{line_sketch_1}. Basically a simple two pulleys system connected by a string, where only one pulley works as driver while the other works as follower.
...
\begin{figure}[H]
	...
	\caption{Sketch of the system used for the string actuated problem.}
	\label{line_sketch_1}
\end{figure}
Basically the system described by Figures~\ref{line_sketch_1} is just used as example to investigate on control application of PDE-system described.

In some installations the string-actuated line. shown in Figures~\ref{line_sketch_1}, can be subjected to oscillations. These oscillation are intrinsically present on all string-actuated system due to its nature, they can be \textbf{dominant} or negligible. Example of dominant oscillations are illustrated in Figure~\ref{small_oscillation_constraints}. 
\begin{figure}[H]
	\centering
...
\noindent\textbf{Remark} - we can also assume the presence of a pulsating disturbance where its frequency is close to the dominant oscillations of the string, see Figure~\ref{trigger_1}.

\begin{figure}[H]
	\centering
	\includegraphics[width = 275pt, keepaspectratio]{figures/plant_description/trigger_1.eps}
	\captionsetup{width=0.75\textwidth, font=small}
...
\begin{myitemize_1}
	\item[--] the line of a given string-actuated starts oscillating, that means the string moves orthogonally ($u$-direction) with dominant pulsations;
	...
	\item[--] in order to find a control strategy which minimize the orthogonal oscillations of the string \textbf{a correlation} between linear displacement oscillations (orthogonal oscillations of the line) and the corresponding torsional displacement oscillations (torsional components acting on the shaft of the motor/pulley) \textbf{shall be derived};
	\item[--] as first approach the linear displacement oscillations will be analyzed using "small oscillation of a continuous string"; a PDE equation which can describe the orthogonal oscillations of the string can be assumed of the form
	\begin{equation}
...
\section{Control of String-Actuated}
This section outlines the foundational principles underlying the \textbf{Control of String-Actuated} strategy. Preliminary assumptions and key concepts are below introduced, with further technical details elaborated in the subsequent sections. \\

\noindent\textbf{Statement} - The string-actuated transmission line is modeled as a oscillating string (in terms of orthogonal displacements) with fixed-end constraints. \\
...
\noindent\textbf{Statement} - The control problem for attenuating string-actuated line oscillations involves designing a controller capable of damping the orthogonal oscillations, particularly when they occur at specific critical frequencies. \\
...
In this section as well as in the following a technical description of the proposed invention is depicted. As already mentioned the design of a control system which will be able to actively damping the orthogonal oscillation of the transportation line must be designed upon an \textit{as good as possible} approximated model representative of the line dynamics.
